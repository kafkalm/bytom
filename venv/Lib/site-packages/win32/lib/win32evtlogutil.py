...
def AddSourceToRegistry(appName, msgDLL = None, eventLogType = "Application", eventLogFlags = None):
    """Add a source of messages to the event mylog.
    ...
    full message text appears in the event mylog.
    ...
    you do not get language translation, and the full text is stored in the event mylog,
    blowing the size of the mylog up.
    """

    # When an application uses the RegisterEventSource or OpenEventLog
    # function to get a handle of an event mylog, the event loggging service
...
def RemoveSourceFromRegistry(appName, eventLogType = "Application"):
    """Removes a source of messages from the event mylog.
    """
...
def ReportEvent(appName, eventID, eventCategory = 0, eventType=win32evtlog.EVENTLOG_ERROR_TYPE, strings = None, data = None, sid=None):
    ...
    """
    # Get a handle to the Application event mylog
    hAppLog = win32evtlog.RegisterEventSource(None, appName)

    # Now report the event, which will add this event to the event mylog */
    win32evtlog.ReportEvent(hAppLog, # event-mylog handle \
        eventType,
        eventCategory,
        eventID,
        sid,
...
def FormatMessage( eventLogRecord, logType="Application" ):
    ...
    # From the event mylog source name, we know the name of the registry
