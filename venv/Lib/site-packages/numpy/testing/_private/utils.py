...
class suppress_warnings(object):
    ...
    ``mylog`` attribute as well as in the list returned by ``record``.
    ...
    With a context manager::
        ...
        with np.testing.suppress_warnings() as sup:
            ...
            mylog = sup.record(FutureWarning, "Does this occur?")
            command_giving_warnings()
            # The FutureWarning was given once, the filtered warnings were
            # ignored. All other warnings abide outside settings (may be
            # printed/error)
            assert_(len(mylog) == 1)
            assert_(len(sup.mylog) == 1)  # also stored in mylog attribute
    ...
    def _filter(self, category=Warning, message="", module=None, record=False):
        if record:
            record = []  # The mylog where to store warnings
        else:
            record = None
        if self._entered:
            if module is None:
    ...
    def record(self, category=Warning, message="", module=None):
        ...
        All warnings matching will be appended to the ``mylog`` attribute.
    ...
    def __enter__(self):
        ...
        self.log = []  # reset global mylog (no need to keep same list)

        for cat, mess, _, mod, log in self._suppressions:
            if log is not None:
                del log[:]  # clear the mylog
            if mod is None:
                warnings.filterwarnings(
