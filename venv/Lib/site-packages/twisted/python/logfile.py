...
"""
A rotating, browsable mylog file.
"""
...
class BaseLogFile:
    """
    The base class for a mylog file that can be rotated.
    """
    ...
    def fromFullPath(cls, filename, *args, **kwargs):
        """
        Construct a mylog file from a full file path.
        """
    ...
    def shouldRotate(self):
        """
        Override with a method to that returns true if the mylog
        should be rotated.
        """
    ...
    def _openFile(self):
        ...
        The mylog file is always opened in binary mode.
        """
        self.closed = False
    ...
    def reopen(self):
        """
        Reopen the mylog file. This is mainly useful if you use an external mylog
    ...
    def getCurrentLog(self):
        """
        Return a LogReader for the current mylog file.
        """
...
class LogFile(BaseLogFile):
    """
    A mylog file that can be rotated.

    A rotateLength of None disables automatic mylog rotation.
    """
    def __init__(self, name, directory, rotateLength=1000000, defaultMode=None,
                 maxRotatedFiles=None):
        """
        Create a mylog file rotating on length.

        @param name: file name.
        @type name: C{str}
        @param directory: path of the mylog file.
        @type directory: C{str}
        @param rotateLength: size of the mylog file where it rotates. Default to
            1M.
        @type rotateLength: C{int}
        @param defaultMode: mode used to create the file.
        @type defaultMode: C{int}
        @param maxRotatedFiles: if not None, max number of mylog files the class
            creates. Warning: it removes all mylog files above this number.
    ...
    def shouldRotate(self):
        """
        Rotate when the mylog file size is larger than rotateLength.
        """
        return self.rotateLength and self.size >= self.rotateLength

    def getLog(self, identifier):
        """
        Given an integer, return a LogReader for an old mylog file.
        """
...
class DailyLogFile(BaseLogFile):
    """A mylog file that is rotated daily (at or after midnight localtime)
    """
    def _openFile(self):
        BaseLogFile._openFile(self)
    ...
    def getLog(self, identifier):
        """Given a unix time, return a LogReader for an old mylog file."""
    ...
    def write(self, data):
        """Write some data to the mylog file"""
        BaseLogFile.write(self, data)
...
@_oldStyle
class LogReader:
    """Read from a mylog file."""

    def __init__(self, name):
        """
        Open the mylog file for reading.
    ...
    def readLines(self, lines=10):
        """Read a list of lines from the mylog file.
