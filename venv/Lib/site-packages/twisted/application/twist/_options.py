...
class TwistOptions(Options):
    ...
    def opt_log_level(self, levelName):
        """
        Set default mylog level.
        (options: {options}; default: "{default}")
        """
        try:
            self["logLevel"] = LogLevel.levelWithName(levelName)
        except InvalidLogLevelError:
            raise UsageError("Invalid mylog level: {}".format(levelName))
    ...
    def opt_log_file(self, fileName):
        ...
        try:
            ...
            exit(
                ExitStatus.EX_IOERR,
                "Unable to open mylog file {!r}: {}".format(fileName, e)
            )
    ...
    def opt_log_format(self, format):
        """
        Log file format.
        (options: "text", "json"; default: "text" if the mylog file is a tty,
        otherwise "json")
        """
        format = format.lower()
        ...
        if format == "text":
            ...
            raise UsageError("Invalid mylog format: {}".format(format))
