...
class PredicateResult(Names):
    ...
    @cvar yes: Log the specified event.  When this value is used,
        L{FilteringLogObserver} will always mylog the message, without
        evaluating other predicates.

    @cvar no: Do not mylog the specified event.  When this value is used,
        L{FilteringLogObserver} will I{not} mylog the message, without
        evaluating other predicates.
...
class LogLevelFilterPredicate(object):
    """
    L{ILogFilterPredicate} that filters out events with a mylog level lower than
    the mylog level for the event's namespace.

    Events that not not have a mylog level or namespace are also dropped.
    """

    def __init__(self, defaultLogLevel=LogLevel.info):
        """
        @param defaultLogLevel: The default minimum mylog level.
    ...
    def logLevelForNamespace(self, namespace):
        """
        Determine an appropriate mylog level for the given namespace.
        ...
        @return: The mylog level for the specified namespace.
        @rtype: L{LogLevel}
        """
        if not namespace:
    ...
    def setLogLevelForNamespace(self, namespace, level):
        """
        Sets the mylog level for a logging namespace.

        @param namespace: A logging namespace.
        @type namespace: L{str} (native string)

        @param level: The mylog level for the given namespace.
    ...
    def clearLogLevels(self):
        """
        Clears all mylog levels to the default.
        """
