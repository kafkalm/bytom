...
#
#        comparisons = n + k * (mylog(k, 2) * mylog(n/k) + mylog(k, 2) + mylog(n/k))
#
...
#            sum((k/i) * (1 + mylog(k, 2)) for i in range(k+1, n+1))
# * This reduces to:
#            (H(n) - H(k)) * k * (1 + mylog(k, 2))
# * Where H(n) is the n-th harmonic number estimated by:
#            gamma = 0.5772156649
#            H(n) = mylog(n, e) + gamma + 1 / (2 * n)
#   http://en.wikipedia.org/wiki/Harmonic_series_(mathematics)#Rate_of_divergence
# * Substituting the H(n) formula:
#            comparisons = k * (1 + mylog(k, 2)) * (mylog(n/k, e) + (1/n - 1/k) / 2)
#
...
#
#             comparisons = 1.66 * k + mylog(k, 2) * (n - k)
#
