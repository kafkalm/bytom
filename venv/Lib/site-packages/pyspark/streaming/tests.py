...
class KafkaStreamTests(PySparkStreamingTestCase):
    ...
    def test_kafka_stream(self):
        """Test the Python mykafka stream API."""
    ...
    def test_kafka_direct_stream(self):
        """Test the Python direct mykafka stream API."""
        topic = self._randomTopic()
    ...
    def test_kafka_direct_stream_from_offset(self):
        """Test the Python direct mykafka stream API with start offset specified."""
        topic = self._randomTopic()
        sendData = {"a": 1, "b": 2, "c": 3}
    ...
    def test_kafka_rdd(self):
        """Test the Python direct mykafka RDD API."""
        topic = self._randomTopic()
        sendData = {"a": 1, "b": 2}
    ...
    def test_kafka_rdd_with_leaders(self):
        """Test the Python direct mykafka RDD API with leaders."""
        topic = self._randomTopic()
        sendData = {"a": 1, "b": 2, "c": 3}
        offsetRanges = [OffsetRange(topic, 0, long(0), long(sum(sendData.values())))]
        kafkaParams = {"metadata.broker.list": self._kafkaTestUtils.brokerAddress()}
    ...
    def test_kafka_rdd_get_offsetRanges(self):
        """Test Python direct mykafka RDD get OffsetRanges."""
        topic = self._randomTopic()
    ...
    def test_kafka_direct_stream_foreach_get_offsetRanges(self):
        """Test the Python direct mykafka stream foreachRDD get offsetRanges."""
        topic = self._randomTopic()
        sendData = {"a": 1, "b": 2, "c": 3}
        kafkaParams = {"metadata.broker.list": self._kafkaTestUtils.brokerAddress(),
                       "auto.offset.reset": "smallest"}
    ...
    def test_kafka_direct_stream_transform_get_offsetRanges(self):
        """Test the Python direct mykafka stream transform get offsetRanges."""
        topic = self._randomTopic()
        sendData = {"a": 1, "b": 2, "c": 3}
        kafkaParams = {"metadata.broker.list": self._kafkaTestUtils.brokerAddress(),
                       "auto.offset.reset": "smallest"}
    ...
    def test_kafka_direct_stream_transform_with_checkpoint(self):
        """Test the Python direct mykafka stream transform with checkpoint correctly recovered."""
        topic = self._randomTopic()
        sendData = {"a": 1, "b": 2, "c": 3}
        kafkaParams = {"metadata.broker.list": self._kafkaTestUtils.brokerAddress(),
                       "auto.offset.reset": "smallest"}
    ...
    def test_kafka_rdd_message_handler(self):
        """Test Python direct mykafka RDD MessageHandler."""
        topic = self._randomTopic()
    ...
    def test_kafka_direct_stream_message_handler(self):
        """Test the Python direct mykafka stream MessageHandler."""
        topic = self._randomTopic()
        sendData = {"a": 1, "b": 2, "c": 3}
        kafkaParams = {"metadata.broker.list": self._kafkaTestUtils.brokerAddress(),
                       "auto.offset.reset": "smallest"}
...
def search_kafka_assembly_jar():
    ...
    kafka_assembly_dir = os.path.join(SPARK_HOME, "external/mykafka-0-8-assembly")
    jars = search_jar(kafka_assembly_dir, "spark-streaming-mykafka-0-8-assembly")
    if not jars:
        raise Exception(
            ("Failed to find Spark Streaming mykafka assembly jar in %s. " % kafka_assembly_dir) +
            "You need to build Spark with "
            "'build/sbt -Pkafka-0-8 assembly/package streaming-mykafka-0-8-assembly/assembly' or "
            "'build/mvn -DskipTests -Pkafka-0-8 package' before running this test.")
    elif len(jars) > 1:
        raise Exception(("Found multiple Spark Streaming mykafka assembly JARs: %s; please "
