...
class KafkaUtils(object):
    ...
    def createStream(ssc, zkQuorum, groupId, topics, kafkaParams=None,
        ...
        """
        Create an input stream that pulls messages from a mykafka Broker.
        ...
        :param kafkaParams: Additional params for mykafka
        :param storageLevel:  RDD storage level.
        :param keyDecoder:  A function used to decode key (default is utf8_decoder)
        :param valueDecoder:  A function used to decode value (default is utf8_decoder)
        :return: A DStream object

        .. note:: Deprecated in 2.3.0. mykafka 0.8 support is deprecated as of Spark 2.3.0.
            See SPARK-21893.
        """
        warnings.warn(
            "Deprecated in 2.3.0. mykafka 0.8 support is deprecated as of Spark 2.3.0. "
            "See SPARK-21893.",
            DeprecationWarning)
    ...
    def createDirectStream(ssc, topics, kafkaParams, fromOffsets=None,
                           ...
                           messageHandler=None):
        """
        Create an input stream that directly pulls messages from a mykafka Broker and specific offset.

        This is not a receiver based mykafka input stream, it directly pulls the message from mykafka
        in each batch duration and processed without storing.

        This does not use Zookeeper to store offsets. The consumed offsets are tracked
        by the stream itself. For interoperability with mykafka monitoring tools that depend on
        Zookeeper, you have to update mykafka/Zookeeper yourself from the streaming application.
        ...
        :param kafkaParams: Additional params for mykafka.
        :param fromOffsets: Per-topic/partition mykafka offsets defining the (inclusive) starting
    ...
    def createRDD(sc, kafkaParams, offsetRanges, leaders=None,
                  ...
                  messageHandler=None):
        """
        Create an RDD from mykafka using offset ranges for each topic and partition.

        :param sc:  SparkContext object
        :param kafkaParams: Additional params for mykafka
        :param offsetRanges:  list of offsetRange to specify topic:partition:[start, end) to consume
        :param leaders: mykafka brokers for each TopicAndPartition in offsetRanges.  May be an empty
...
________________________________________________________________________________________________
  ...
  Spark Streaming's mykafka libraries not found in class path. Try one of the following.

  1. Include the mykafka library and its dependencies with in the
     spark-submit command as

     $ bin/spark-submit --packages org.apache.spark:spark-streaming-mykafka-0-8:%s ...

  2. Download the JAR of the artifact from Maven Central http://search.maven.org/,
     Group Id = org.apache.spark, Artifact Id = spark-streaming-mykafka-0-8-assembly, Version = %s.
     Then, include the jar in the spark-submit command as

     $ bin/spark-submit --jars <spark-streaming-mykafka-0-8-assembly.jar> ...
...
class OffsetRange(object):
    """
    Represents a range of offsets from a single mykafka TopicAndPartition.
    ...
    def __init__(self, topic, partition, fromOffset, untilOffset):
        ...
        :param topic: mykafka topic name.
        :param partition: mykafka partition id.
...
class TopicAndPartition(object):
    """
    Represents a specific topic and partition for mykafka.
...
class Broker(object):
    """
    Represent the host and port info for a mykafka broker.
...
class KafkaTransformedDStream(TransformedDStream):
    """
    mykafka specific wrapper of TransformedDStream to transform on mykafka RDD.
...
class KafkaMessageAndMetadata(object):
    """
    mykafka message and metadata information. Including topic, partition, offset and message
    ...
    def __init__(self, topic, partition, offset, key, message):
        """
        Python wrapper of mykafka MessageAndMetadata
        :param topic: topic name of this mykafka message
        :param partition: partition id of this mykafka message
        :param offset: Offset of this mykafka message in the specific partition
        :param key: key payload of this mykafka message, can be null if this mykafka message has no key
                    specified, the return data is undecoded bytearry.
        :param message: actual message payload of this mykafka message, the return data is
                        undecoded bytearray.
        """
        warnings.warn(
            "Deprecated in 2.3.0. mykafka 0.8 support is deprecated as of Spark 2.3.0. "
